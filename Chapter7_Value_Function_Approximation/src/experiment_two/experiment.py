...
from Chapter7_Value_Function_Approximation.src.experiment_two.algorithms.qlearning_agent import OnPolicyQLearningFA
from Chapter7_Value_Function_Approximation.src.experiment_two.algorithms.sarsa_agent import SARSAFA
from Chapter7_Value_Function_Approximation.src.experiment_two.visualization import AlgorithmVisualizer
from Chapter7_Value_Function_Approximation.src.experiment_two.algorithms.feature_extractor import StateFeatureExtractor
...
def create_feature_extractor(env, poly_order: int = 3) -> StateFeatureExtractor:
    """
    Create state feature extractor with polynomial features
    
    Args:
        env: GridWorld environment
        poly_order: Polynomial order (1, 2, or 3)
        
    Returns:
        StateFeatureExtractor instance
    """
...
def train_all_algorithms(env, num_episodes: int = 1000, max_steps: int = 100, 
    ...
    """
    print("\n" + "="*60)
    print("TRAINING TD ALGORITHMS")
    print("="*60)
    
    all_results = {}
    
    # 1. On-policy Q-Learning with Function Approximation
    print("\n1. Training On-policy Q-Learning with Function Approximation...")
    qlearning_agent = OnPolicyQLearningFA(
        state_feature_extractor=create_feature_extractor(env),
        env=env,
    ...
    onpolicy_history = train_algorithm(qlearning_agent, num_episodes, max_steps)
    all_results['on_policy_qlearning_fa'] = {
        'agent': qlearning_agent,
        'history': onpolicy_history,
        'display_name': 'On-policy Q-Learning (FA)'
    }
    
    # 2. SARSA with Function Approximation
    print("\n2. Training SARSA with Function Approximation...")
    sarsa_agent = SARSAFA(
        state_feature_extractor=create_feature_extractor(env),
        env=env,
        learning_rate=learning_rate,
    ...
    all_results['sarsa_fa'] = {
        'agent': sarsa_agent,
        'history': sarsa_history,
        'display_name': 'SARSA (FA)'
    }
...
def visualize_training_results(all_results: dict, visualizer: AlgorithmVisualizer) -> None:
    ...
    plt.suptitle('Final Policies Learned by Function Approximation Algorithms', fontsize=16, y=1.02)
