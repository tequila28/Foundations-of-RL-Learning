"""
On-line Q-Learning Algorithm Implementation
On-line learning with immediate updates after each step, using Q-learning update rule.
"""
...
class OnPolicyQLearning:
    """On-line Q-Learning algorithm implementation (immediate updates)."""
    ...
    def __init__(self, 
        ...
        """
        Initialize On-line Q-Learning algorithm.
    ...
    def train(self, 
        ...
        """
        Train the on-line Q-learning agent.
        ...
        """
        for episode in tqdm(range(num_episodes), desc="Training On-policy Q-Learning"):
    ...
    def calculate_episode_td_error(self, episode_transitions: List[Tuple[int, int, float, int]]) -> float:
        ...
        for state, action, reward, next_state in episode_transitions:
            current_q = self.Q[state, action]
            max_next_q = np.max(self.Q[next_state]) if next_state not in self.env.target_states else 0
            td_target = reward + self.gamma * max_next_q
    ...
    def generate_episode(self, max_steps: int = 100) -> Tuple[List[Tuple[int, int, float, int]], int]:
        """
        Generate an episode by following current policy (for analysis purposes).
        
        Args:
            max_steps: Maximum steps per episode
            
        Returns:
            Tuple of (episode transitions, number of steps)
        """
        state = 0
        episode_transitions = []
        steps = 0
        ...
        return episode_transitions, steps